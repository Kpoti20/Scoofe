  ...
  if(isset($_POST['forminscription']))
{
  ...
  if(!empty($pse) AND !empty($mail) AND !empty($mdp))
   
  {
     $mdpe = sha1($mdp);
    $pseudolength = strlen($pse);
    //$pseudo = test_input($_POST["pseudo"]);
    if($pseudolength <= 35)
    {
      if (preg_match("/^[a-zA-Z ]*$/",$pse)) 
      {
          if(filter_var($mail,FILTER_VALIDATE_EMAIL))
          {
            ...
            { 
             ...
             $insertmbr->execute( array($pse,$mail,$mdpe)); 
                "<br/>";
                $erreur="Utilisateur crée avec succes!"; 
              }else{
                  $erreur = "Desole vous n avez pas le droit de creer un utilisateur ";
              }
            } 
            else
            {
    ...
    {
      $erreur="votre pseudo ne dois pas depasser 35 caracteres!!!";
    }
  }
  else
  {
...
<?php 
      include_once 'etablissement_parametre.php';
    ?>
<!-- Page Container -->
<div class="w3-content w3-margin-top" style="max-width:1400px;">
  ...
  <div class="w3-row-padding">
    ...
    <div class="w3-twothird">
        ...
        <h2 class="w3-text-teal w3-padding-4" style="margin-left:30px;"><i>Nouveau Utilisateur</i></h2>
        <div class="w3-container">
          <div class="w3-row-padding w3-light-grey w3-padding-16 w3-container">
          <div class="w3-content">
            <form method="POST" action="">
              <p>
              <label for="pse" >Pseudo</label>
              <input class="w3-input "  autocomplete="off" type="text" value="<?php if (isset($pse)) {echo $pse;} ?>" placeholder="Pseudo" id="pse" name="pse"></p>
              ...
              <input class="w3-input" type="password"  autocomplete="off"  placeholder="Votre mot de passe" id="mdp" name="mdp"  ></p>
      ...
      <div class="w3-container w3-card-2 ">
        <h2 class="w3-text-teal w3-padding-16" style="margin-left:15px;"><i>Liste des Utilisateurs</i></h2>
    <?php   
    $bdd = new PDO('mysql:host=127.0.0.1; dbname=scolariteweb','root', '');
    $s=$_SESSION['id'];
    if (isset($_SESSION['niv']) AND $userinfo['niv'] == 2 ) {

      $_select = $bdd->prepare("SELECT * FROM etablissement WHERE id='$s' ");
    } else {
      $_select = $bdd->prepare("SELECT * FROM etablissement ");
    }
...
?>    
      <tr>
        <td><?php echo $s->id;?></td>
        <td><?php echo $s->pseudo;?></td>
        <td><?php echo $s->mail;?></td>
        <td>
          <a href="etablissement_parametre_action.php?id=<?php echo $s->id;?>" target="_parent"  style="text-decoration:none;">Modifier</a>
        </td>
      </tr> 
          <?php
