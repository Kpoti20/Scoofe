  ...
  if(isset($_POST['valider']))
{
            ...
            $mont = $_POST['mont'];
     if(!empty($ettu)  AND !empty($mont))

      {
                
                $mtl = strlen($mont);
                   if($ettu!="Desole aucun étudiant ne correspond à votre recherche")
                  {
                     ...
                     if($mtl>0)
                    {
                         ...
                         $_update = $bdd->prepare("UPDATE payement SET  mate='$ettu', mont='$mont'  WHERE idp=$idp ");
                            $_update->execute();
                            $erreur="Modification effectuée avec succes! ";
                     }
                      else
                      {
                        $erreur="SVP Veuillez saisir le montant restant correct et different de zero";
                      } 
              }else
                {
                  $erreur="Veuillez choisir un étudiant !";
                } 
              }else{
          ...
          if(isset($_POST['supprimer']))
           {
           $idp=$_POST['idp'];
           $_delete = $bdd->prepare("DELETE  FROM payement WHERE idp=$idp");
           $_delete->execute();
...
<?php 
      include_once 'etablissement_payement_action.php';
    ?>
<!-- Page Container -->
  ...
  <div class="w3-row-padding">
    ...
    <div class="w3-twothird">
      ...
      <div class="w3-container w3-card-2  w3-margin-bottom">
        <h2 class="w3-text-teal w3-padding-4" style="margin-left:30px;"><i>Modification du payement de l'étudiant : <?php echo $s->mate;?> </i></h2>
        <div class="w3-container">
          ...
          <div class="w3-content">
            ...
            <form method="POST" action="">
             ...
             <p>
          <input type="hidden"  value="<?php echo $s->idp; ?>" readonly="true" name="idp">
          </p>
          <label>Etudiant</label>
         <input class="w3-input" value="<?php echo $s->mate;?>" name="etu" readonly="true" id="etu" type="text" autocomplete="off" placeholder="Selectionnez un étudiant"><div id="etudiantList"></div>
          </p>
          
          <p>
          <label>Montant</label>
          <input class="w3-input" value="<?php echo $s->mont;?>" autocomplete="off" name="mont" type="number"></p>
      ...
      <div class="w3-container w3-card-2 ">
        <h2 class="w3-text-teal w3-padding-16" style="margin-left:15px;"><i>Suppression du payement de l'étudiant : <?php echo $z->mate;?> </i></h2>
    <p style="margin-left:15px;">
      A moins que vous ne trouviez plus l'importance du payement de cet étudiant dans votre liste de payement , vous avez la possibilité de <a href="#demo" style="text-decoration:none; cursor:pointer;" data-toggle="collapse">supprimer le payement de l'étudiant en question</a>
    </p>
    <div id="demo" style="margin-left:15px;" class="collapse">
    Voulez-vous réellement supprimer l'étudiant : <?php echo $z->mate; ?>
      <form method="POST" action="">
        <input type="hidden"  value="<?php echo $z->idp; ?>" readonly="true" name="idp">
